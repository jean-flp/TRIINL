    ...
    ERC1155Supply
{
    ...
    function mint(
        ...
        books[id] = Book(title, author, isbn, ano, uriSuffix, msg.sender);
...
function _asSingletonArray(uint256 element) private pure returns (uint256[] memory) {
    ...
    // ) internal override(ERC1155, ERC1155Pausable, ERC1155Supply) {
    //     if (from != address(0) && to != address(0)) {
    //         // Allow library-to-user transfers (loan approval)
    //         // Allow user-to-library transfers (loan return)
    //         // Allow library-to-library transfers
    //         bool isLibraryToUser = hasRole(LIBRARY_ROLE, from) &&
    //             hasRole(USER_ROLE, to);
    //         bool isUserToLibrary = hasRole(USER_ROLE, from) &&
    //             hasRole(LIBRARY_ROLE, to);
    //         bool isLibraryToLibrary = hasRole(LIBRARY_ROLE, from) &&
    //             hasRole(LIBRARY_ROLE, to);

    //         if (!(isLibraryToUser || isUserToLibrary || isLibraryToLibrary)) {
    ...
    //                 "Book does not exist"
    //             );
    //             // Ensure user-to-library transfers are only to the original library
    //             if (isUserToLibrary) {
    //                 require(
...
//     error InvalidInput(string field);
//     error UnauthorizedTransfer(address from, address to);
//     error InvalidReturn(string reason);
...
//             bytes(enderecoEmail).length == 0
//         ) {
//             revert InvalidInput("name, sigla or email cannot be empty");
//         }
...
//             bytes(uriSuffix).length == 0
//         ) {
//             revert InvalidInput("all book fields must be non-empty");
//         }
...
//     ) external onlyRole(DEFAULT_ADMIN_ROLE) {
//         require(
//             libraries[libraryAddress].isActive,
//             "Library is already inactive or does not exist"
//         );
//         libraries[libraryAddress].isActive = false;
//         _revokeRole(LIBRARY_ROLE, libraryAddress);
